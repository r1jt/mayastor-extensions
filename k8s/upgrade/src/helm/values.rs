use crate::{
    common::{
        constants::{
            TWO_DOT_EIGHT, TWO_DOT_FIVE, TWO_DOT_FOUR, TWO_DOT_NINE, TWO_DOT_ONE, TWO_DOT_O_RC_ONE,
...
";
    ...
    // Special-case values for 2.9.0.
    if source_version.ge(&two_dot_o_rc_one) && source_version.lt(&TWO_DOT_NINE) {
        // {{.Values.etcd.service.port}} --> {{ default .Values.etcd.service.ports.client .Values.etcd.service.port }}
        if source_values
            .base_init_containers()
            .iter()
            .filter_map(|container| container.command.clone())
            .flatten()
            .any(|cmd| cmd == "{{.Values.etcd.service.port}}")
        {
            let init_containers_key = YamlKey::try_from(".base.initContainers.containers")?;
        ...
        {
            yq.delete_object(
                YamlKey::try_from(".etcd.client.secureTransport")?,
                upgrade_values_file.path(),
            )?;
            yq.set_unquoted_value(
        ...
        {
            yq.delete_object(
                YamlKey::try_from(".etcd.peer.secureTransport")?,
                upgrade_values_file.path(),
            )?;
            yq.set_unquoted_value(
        ...
        if let Some(reclaim_policy) = source_values.etcd_deprecated_reclaim_policy() {
            yq.delete_object(
                YamlKey::try_from(".etcd.persistence.reclaimPolicy")?,
                upgrade_values_file.path(),
            )?;
            yq.set_quoted_string_value(
                YamlKey::try_from(".etcd.persistentVolumeClaimRetentionPolicy.whenDeleted")?,
                reclaim_policy,
                upgrade_values_file.path(),
            )?;
            yq.set_quoted_string_value(
                YamlKey::try_from(".etcd.persistentVolumeClaimRetentionPolicy.whenScaled")?,
                reclaim_policy,
                upgrade_values_file.path(),
            )?;
        }
